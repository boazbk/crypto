...
The original currencies were based on [commodity money](https://goo.gl/K7awAW). The scarce resource was some commodity having intrinsic value, such as gold or silver, or even salt or tea, and ownership based on physcial posession.
...
In contrast if they "fork" the consensus ledger then their work may welll be for vain.
...
> # {.lemma #randomcrhlem}
If $H$ is a random function from some domain $S$ to $\{0,1\}^n$, then the probabilisty that after $T$ queries an attacker
