...
Most people, once they understand what the definition says, don't like it that much initially.
...
to some related $c'$.
Another way to say it is that we need to ensure the _integrity_ of messages to ensure their _confidentiality_ if we want to handle _active_ adversaries that might modify messages on the channel.
...
Let $(E,D)$ be CPA-secure encryption scheme and $(S,V)$ be a CMA-secure MAC with $n$ bit keys.
...
where the original decryption box would not have done so, but this happens with probability at most $\epsilon/10$. Hence, if $M'$ has success $1/2+\epsilon$ in the CCA game, then even if it's the case that $M'$ always outputs the wrong answer when $Eve$ makes this mistake, we will still get success at least $1/2+0.8\epsilon$, hence contradicting the CPA security of $(E,D)$ and concluding the proof of the theorem.
